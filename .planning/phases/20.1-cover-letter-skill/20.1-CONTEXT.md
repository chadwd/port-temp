# Phase 20.1 Context: Cover Letter Generation

## Purpose

This document captures the cover letter structure, best practices, and guidelines for the cover letter generator skill.

Source: `src/data/sources/cover_letter_system_prompt.md`

---

## Core Principle

A cover letter is **not** an autobiography. It is a **marketing document**.

The goal is to demonstrate **clear role fit** and **authentic motivation** in the shortest, clearest way possible.

If the letter sounds generic, self-focused, or like a resume rewrite, it has failed.

---

## Non-Negotiable Rules

- Always write a new cover letter per role
- Never reuse generic phrasing
- Optimize for clarity, relevance, and credibility
- One page maximum
- Plain language, no jargon
- Employer language over candidate language
- Show, do not claim

---

## Operating Workflow

### Step 1: Analyze the Job Description

Break the job description into two buckets:

1. **What the candidate will do**
2. **What the company is looking for**

Rules:
- Prioritize the first few bullets under responsibilities
- Highlight only skills the candidate genuinely has
- Note preferred or nice-to-have qualifications if applicable
- Ignore requirements the candidate does not meet
- Never disqualify the candidate for missing some requirements

Output (internal only):
- A short, prioritized list of required skills and responsibilities

---

### Step 2: Build a Skill-Matching Table (Internal Reasoning)

Create a two-column table internally:

- Column 1: Exact phrases from the job description
- Column 2: How the candidate matches each phrase

Rules:
- Mirror the employer's language exactly
- Be specific and concrete
- Do not worry about writing quality yet
- Only the **top two** strongest matches will be used in the final letter

Purpose:
This step ensures alignment and prevents vague or generic claims.

---

### Step 3: Research the Company

This step is mandatory.

Answer the following:
- What is the company's mission?
- What problem are they solving?
- What product or platform do they offer?
- What differentiates them from competitors?
- What values do they publicly emphasize?
- Any notable blog posts, interviews, or announcements?

Then select:
- One **values-driven reason**
- One **product, industry, or direction-driven reason**

If the candidate uses the product, prioritize that.

---

## Cover Letter Structure (Mandatory)

### Paragraph 1: Identity and Intent

Purpose:
- Establish who the candidate is
- Clarify what role they want
- Signal alignment with the company

Rules:
- 1–2 sentences
- Mention the company by name
- Reference something specific about them

---

### Paragraph 2: Summary and Transition

Purpose:
- Summarize what the candidate brings
- Transition into skill evidence

Rules:
- One sentence summarizing impact
- One sentence setting up skills
- Use numbers or outcomes when possible
- Avoid buzzwords

---

### Paragraphs 3–4: Skill and Qualification Match

Purpose:
- Prove fit through evidence

Process:
- Select the top two skills from the matching table
- Assign each skill a story theme

Approved Story Themes:
- Leading people
- Taking initiative
- Solving challenging problems
- Working across disciplines
- Curiosity and learning
- Managing conflict
- Recovering from failure

Structure per paragraph:
- Name the theme implicitly
- Describe the experience
- Show the outcome or impact

Rules:
- No bullet lists
- No resume repetition
- No unsupported claims

---

### Paragraph 5: Why This Company

Purpose:
- Demonstrate genuine interest

Structure:
- Weave together the two selected reasons

Template logic:
- Value alignment plus personal relevance
- Product or direction plus career relevance

If this section feels forced, the application is misaligned.

---

### Paragraph 6: Conclusion

Purpose:
- Close with confidence

Rules:
- Be direct
- Express readiness
- Invite next steps
- No desperation

---

## Style Guidelines

- Clear and concise
- Professional but human
- Confident, not arrogant
- Specific over clever
- Short sentences preferred

---

## Failure Conditions (Avoid at All Costs)

- Generic openings
- Company-agnostic statements
- Repeating resume bullets
- Overly long explanations
- Buzzwords without evidence

---

## Success Criteria

A successful cover letter:
- Feels tailored to one company only
- Clearly maps skills to role needs
- Explains why the candidate wants *this* job
- Can be read in under 90 seconds

---

## Integration with Job-Matcher Workflow

The cover letter generator integrates with existing job analysis workflow:

1. User runs `/job-matcher [job-posting]` → produces job-analysis file
2. Job-analysis file contains:
   - Role context and pain points
   - Cultural signals
   - Pre-scored accomplishments (weighted algorithm)
   - Match assessment
3. User runs `/cover-letter-generator [job-link] [analysis-file]` → uses job-analysis for:
   - Company research (extracted from analysis)
   - Skill-matching table (uses pre-scored accomplishments)
   - Tailored positioning (based on cultural signals)

This ensures consistency between resume, cover letter, and job match assessment.

---

## Output Format

**File location:** `src/data/cover-letters/[company]-[role]-cover-letter.md`

**Naming convention:** Same as resumes (lowercase, hyphenated)
- Example: `coinbase-senior-staff-product-designer-platform-cover-letter.md`

**Format:** Markdown with minimal frontmatter
```yaml
---
company: [Company Name]
role: [Role Title]
job_posting_url: [URL]
created: [YYYY-MM-DD]
---
```

**Content structure:** 6 paragraphs as defined above, plain text, professional formatting.
